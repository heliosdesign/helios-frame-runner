...
export default class FrameRunner {
  ...
  rafFunction(){
    ...
    for (this.i = this.functionArray.everyFrame.length - 1; this.i >= 0; this.i--)
      this.functionArray.everyFrame[this.i]()

    this.counter++
    if(this.counter>60) this.counter = 0

    if(this.counter === 0) {
      for (this.i = this.functionArray.everySecond.length - 1; this.i >= 0; this.i--)
        this.functionArray.everySecond[this.i]()
    }
